...
mod tests {
    ...
    async fn test_accepted_vote() -> anyhow::Result<()> {
        let candidates = vec![Candidate(String::from("Azleus"))];
        ...
        let vote_form =  VoteForm{
            voter: String::from("Azleus"),
            candidate: String::from("Azleus")
        };
        let mut correct_scores = BTreeMap::new();
		correct_scores.insert(Candidate(String::from("Azleus")), Score(1));
        ...
        assert_eq!(result,VoteOutcome::AcceptedVote(Voter( String::from("Azleus")), Candidate(String::from("Azleus"))));
    ...
    async fn test_blank_vote() -> anyhow::Result<()> {
        ...
        let mut correct_scores = BTreeMap::new();
		correct_scores.insert(Candidate(String::from("Azleus")), Score(0));
    ...
    async fn test_null_vote() -> anyhow::Result<()> {
        ...
        let vote_form =  VoteForm{
            voter: String::from("Azleus"),
            candidate: String::from("Vena Mondlane")
        };
        let mut correct_scores = BTreeMap::new();
		correct_scores.insert(Candidate(String::from("Azleus")), Score(0));
        ...
        let result = voting_controller.vote(vote_form).await.expect("err lors du vote");
        let voting_machine = voting_controller.get_voting_machine().await.expect("erreur lors de la recuperation");
        assert_eq!(result,VoteOutcome::InvalidVote(Voter( String::from("Azleus"))));
        assert_eq!(correct_scoreboard,voting_machine.get_scoreboard().clone());
        Ok(())
    }
    ...
    async fn test_has_already_voted() -> anyhow::Result<()> {
        ...
        let voting_machine = voting_controller.get_voting_machine().await.expect("erreur lors de la recuperation");
        assert_eq!(result,VoteOutcome::HasAlreadyVoted(Voter(String::from("Azleus"))));
        assert_eq!(correct_scoreboard,voting_machine.get_scoreboard().clone());
        Ok(())
    }
}